...
from sub_platforms.sql_optimizer.common.db_variable import VariablesAboutIndex
from sub_platforms.sql_optimizer.common.sample_info import SampleColumnInfo
from sub_platforms.sql_optimizer.databases.mysql.mysql_command import MySQLCommand, get_mysql_version, MySQLVersion
from sub_platforms.sql_optimizer.meta import Table, Column, IndexColumn, IndexType
from sub_platforms.sql_optimizer.videx.videx_mysql_utils import get_mysql_utils, MySQLConnectionConfig, DBTYPE
...
class Env(ABC):
    ...
    @abstractmethod
    def get_sample_data(self, db_name: str, table_name: str, table_meta: Table, sample_cols: Set[SampleColumnInfo],
                        pk_names: List[str],
    ...
    def get_sample_data(self, db_name: str, table_name: str, table_meta: Table, sample_cols: Set[SampleColumnInfo],
        ...
        for col in sample_cols:
            if col.sample_length > 0:
                projections.append(
                    f'LEFT({add_backquote(col.column_name)}, {col.sample_length}) as {add_backquote(col.column_name)}')
            else:
